...
class ConfigFormTemplate(ConfigForm):
    ...
    async def save_config(self, e) -> None:
        ...
        if old_name == self.name_field.value:
            overwrite_decision = "rename"
        else:
            overwrite_decision: bool | None | str = await self.detect_conflict(self.agent.config_store_entries, self.name_field.value, old_name)
            if overwrite_decision == True:
                # method to soft remove from agent setup added configs
                # global_event_bus.publish("soft_remove_config_entry", self.config.tile.key)
                pass
            elif overwrite_decision == False:
                return
