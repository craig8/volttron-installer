...
def main():
    ...
    packages = [
        line.split()[1]
        for line in dpkg_output.decode().splitlines()
        if line.startswith("ii")
    ]

    pexpect_version_cmd = Popen(
        ["bash", "-c", "pip freeze | grep pexpect"], stdout=PIPE, stderr=PIPE
    )
    pexpect_version = pexpect_version_cmd.stdout.read().decode("utf-8")

    nicegui_cmd = Popen(
        ["bash", "-c", "pip freeze | grep nicegui"], stdout=PIPE, stderr=PIPE
    )
    ...
    if not venv_installed:
        print(
            "Python3.10-venv has not been installed. Now installing the python3.10-venv package using apt."
        )
